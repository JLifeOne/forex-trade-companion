...
const AuthModalComponent: React.FC<AuthModalProps> = ({ visible, onClose }) => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (loginWithFirebase.fulfilled.match(resultAction)) {
      ...
      const errorMessage = resultAction.payload as string || 'Login failed. Please try again.';
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    if (signupWithFirebase.fulfilled.match(resultAction)) {
      ...
      const errorMessage = resultAction.payload as string || 'Sign up failed. Please try again.';
      dispatch(addToast({ message: errorMessage, type: 'error' }));
    }
    setIsLoading(false);
  };
